...
for (const needle of ['Weekly Claw Episodes', 'W29', 'data-video-id="-M4zet86U_A"', '/assets/youtube-thumbnails/w29-approved-a.png', 'W27', 'W26', 'The agent owns the loop', 'The Sandbox Failed', '/episodes/27/deck', '/episodes/22/deck', 'data-video-id="vqUkh8w2L8g"', 'data-video-id="f2yugYwXOBo"', 'data-video-id="dquJyEBQWpE"', 'data-video-id="MSRFmpDfaTg"', 'data-spotify-id="5t1xYX7e4DUv6l9DKUAi21"', 'data-spotify-id="58kOYSrhrMLArnNfVY41RQ"', '/assets/youtube-thumbnails/w27.png', '/assets/youtube-thumbnails/w22-v2-the-sandbox-failed-approved-20260727.jpg', '/assets/youtube-thumbnails/w21-v2-approved-20260727.jpg', '/assets/youtube-thumbnails/w20-v2-ai-got-cheap-approved-20260727.jpg', '<strong>10</strong>', 'video episodes']) {
...
if (summaryCount !== 10) {
  console.error(`Episodes index should render 10 video-era episode summaries, found ${summaryCount}`);
  process.exit(1);
}
...
if (feedItemCount !== 10) {
  console.error(`RSS feed should contain 10 items, found ${feedItemCount}`);
  process.exit(1);
}
const episodesJson = JSON.parse(readFileSync(new URL('../episodes.json', import.meta.url), 'utf8'));
if (episodesJson.episodes.length !== 10) {
...
if (latestCtas.length !== 3 || latestCtas.some((url) => url !== 'https://www.youtube.com/watch?v=-M4zet86U_A')) {
